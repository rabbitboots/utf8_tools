...
self:registerJob("utf8Conv.utf8_latin1", function(self)
	...
	do
		self:print(2, "[+] arg #1 empty string")
		local ok, err_i, err = utf8Conv.utf8_latin1("")
		self:print(4, ok, err_i, err)
		self:isEqual(ok, "")
	end

	do
		self:print(2, "[-] arg #1 has code points that are unsupported in Latin 1, and we didn't provide a stand-in string.")
		local ok, err_i, err = utf8Conv.utf8_latin1("a灷b灷c灷")
		self:print(4, ok, err_i, err)
		self:isEvalFalse(ok)
	end
	...
	do
		self:print(2, "[+] general UTF-8 to Latin 1 test.")
		local ok, err_i, err = utf8Conv.utf8_latin1(sample1_utf8)
		self:print(4, ok, err_i, err)
...
self:registerJob("utf8Conv.latin1_utf8", function(self)
	...
	do
		self:print(2, "[+] convert Latin 1 to UTF-8")
		local ok, err_i, err = utf8Conv.latin1_utf8(sample1_latin1)
		self:print(4, ok, err_i, err)
...
self:registerJob("utf8Conv.utf16_utf8", function(self)
	...
	do
		self:print(2, "[-] String is too short to hold any valid UTF-16 data.")
		local ok, err_i, err = utf8Conv.utf16_utf8(string.char(0x00))
		self:print(4, ok, err_i, err)
		self:isEvalFalse(ok)
	end

	do
		self:print(2, "[+] Empty string in -> empty string out.")
		local ok, err_i, err = utf8Conv.utf16_utf8("")
		self:print(4, ok, err_i, err)
		self:isEqual(ok, "")
	end

	do
		self:print(2, "[-] first integer of a surrogate pair is out of range (greater than 0xdbff)")
		local ok, err_i, err = utf8Conv.utf16_utf8(string.char(0xff, 0xff, 0x00, 0xdc))
		self:print(4, ok, err_i, err)
		self:isEvalFalse(ok)
	end

	do
		self:print(2, "[-] input string is too short for a surrogate pair.")
		local ok, err_i, err = utf8Conv.utf16_utf8(string.char(0xff, 0xdb), false)
		self:print(4, ok, err_i, err)
		self:isEvalFalse(ok)
	end

	do
		self:print(2, "[-] second integer of a surrogate pair is out of range (0xdc00 - 0xdfff).")
		local ok, err_i, err = utf8Conv.utf16_utf8(string.char(0xff, 0xdb, 0xff, 0xff))
		self:print(4, ok, err_i, err)
		self:isEvalFalse(ok)
	end

	do
		self:print(2, "[+] Convert test string from UTF-16LE to UTF-8.")
		local ok, err_i, err = utf8Conv.utf16_utf8(test_str_utf16le, false)
		self:print(4, ok, err_i, err)
		self:isEqual(ok, test_str_utf8)
	end

	do
		self:print(2, "[+] Convert test string from UTF-16BE to UTF-8.")
		local ok, err_i, err = utf8Conv.utf16_utf8(test_str_utf16be, true)
		self:print(4, ok, err_i, err)
		self:isEqual(ok, test_str_utf8)
	end

	do
		self:print(2, "[-] Convert test string, but mix up the UTF-16 endianness.")
		local ok, err_i, err = utf8Conv.utf16_utf8(test_str_utf16le, true)
		self:print(4, ok, err_i, err)
...
self:registerJob("utf8Conv.utf8_utf16", function(self)
	...
	do
		self:print(2, "[-] Bad input UTF-8.")
		local ok, err_i, err = utf8Conv.utf8_utf16(string.char(0xff))
		self:print(4, ok, err_i, err)
		self:isEvalFalse(ok)
	end
	...
	do
		self:print(2, "[+] Convert from UTF-8 to UTF-16LE.")
		local ok, err_i, err = utf8Conv.utf8_utf16(test_str_utf8, false)
		self:print(4, ok, err_i, err)
	...
	do
		self:print(2, "[+] Convert from UTF-8 to UTF-16BE.")
		local ok, err_i, err = utf8Conv.utf8_utf16(test_str_utf8, true)
		self:print(4, ok, err_i, err)
